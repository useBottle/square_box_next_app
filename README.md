...
## 프로젝트 소개
Square Box 는 사용자가 뉴스, 유튜브 컨텐츠를 검색하고 저장하고 싶은 컨텐츠는 북마크할 수 있는 애플리케이션입니다. 기존의 React 로 개발했던 프로젝트를 Next.js 를 사용해 다시 개발하면서 사용 성능 개선 및 리디자인한 프로젝트입니다.
...
- 좀 더 유연한 스타일 코드 적용을 위해 Emotion 채용.
...
현재 웹 이용자 비율이 모바일 유저가 훨씬 많다는 점을 고려해 모바일 친화 디자인을 적용하기 위해 노력했으며 이에 맞춰 아래 각 페이지에 대한 이미지 또한 모바일 뷰를 중심으로 사용했습니다.
...
- 실시간 검색어를 클릭하면 뉴스 컨텐츠 제공 페이지로 이동하여 검색된 관련 컨텐츠를 제공합니다.
- 실시간 검색어를 통한 데이터 요청 시간을 단축하기 위해 유튜브 데이터 검색이 동시에 이루어지던 것은 제거하고 뉴스 컨텐츠만 요청되도록 변경하였습니다.
- 최신 뉴스 리스트는 기존 프로젝트에는 없던 신규로 추가한 컨텐츠입니다.
- ISR 을 적용해 애플리케이션 초기 접속 시 빌드 타임에 재검증된 최신 데이터를 불러와 렌더링하며 이후는 클라이언트 측에서 주기적으로 서버로부터 최신 데이터를 재요청하여 받아와 리렌더링합니다.
...
- NextAuth.js 를 사용하여 SNS 로그인 기능을 추가하였습니다.
...
- 로그인을 했을 경우 북마크 버튼을 클릭하면 컨텐츠를 북마크하여 북마크 페이지에서 다시 볼 수 있습니다.
- 유튜브 영상 플레이어 페이지는 뉴스와 마찬가지로 Next.js 의 다이나믹 라우트 기능을 이용하여 하나의 컴포넌트에서 여러 컨텐츠를 동적으로 렌더링할 수 있도록 하였습니다.
...
- 사용자가 검색 요청을 한 경우 데이터가 완전히 불러와지기 전까지 Skeleton UI 컴포넌트가 렌더링 되도록 적용하였습니다.
- 웹 스크래핑을 통해 데이터를 가져오기 때문에 유튜브 보다 상대적으로 데이터 응답 속도가 느린 뉴스 컨텐츠 검색 및 상세 페이지에 접속할 경우에 렌더링되도록 했습니다.
...
- SEO 향상을 위해 뉴스 및 유튜브 상세 페이지의 URL 은 쿼리 스트링으로 title 값을 포함하도록 하였고 이에 따라 해당 페이지에서는 시멘틱 태그 사용 및 반응형 웹 디자인에 좀 더 신경을 썼습니다.
- 그에 따른 결과로 기존 프로젝트보다 접근성 및 표준 권장사항 수치가 향상되었습니다.
- Lighthouse 지표 외에 실질적으로 사용자가 특정 컨텐츠를 검색 시 데이터 로딩 시간을 단축하기 위해 로직을 분할 하는 등의 노력을 한 결과, 기존 리액트 프로젝트에서는 평균 16~20초 가까이 소요되던 컨텐츠 로딩 시간이 평균 3~6초 정도로 단축되었습니다.
...
- 모바일 퍼스트 디자인 방식으로 반응형 웹을 적용하였습니다.
- 각 기기에서 볼 때 전체적인 UI 는 비슷하지만 세부적인 UI 요소들이 해당 뷰포트에서 좀 더 나은 UX 를 제공하도록 고려하여 반응형이 적용되도록 설계하였습니다.
- 데스크탑 뷰에서는 뷰포트의 너비를 제한하여 사용자의 시선이 화면 전체로 너무 분산되지 않고 컨텐츠에 집중되도록 하였습니다.
