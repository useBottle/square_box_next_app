...
- Lighthouse 지표 외에 실질적으로 사용자가 특정 컨텐츠를 검색 시 데이터 로딩 시간을 단축하기 위해 로직을 분할 하는 등의 노력을 한 결과, 기존 리액트 프로젝트에서는 사용자의 컨텐츠 검색에 대한 데이터 로딩 시간이 평균 16초에서 20초 가까이 소요되던 것에 비해 평균 3초에서 6초 정도로 단축되었습니다.
- 애플리케이션 최초 접속 시에 데이터 로딩에 대한 클라이언트의 리소스 소모를 줄이고자 기존 프로젝트 (CSR) 대비 SSR 로 전환하였을 때는 오히려 성능 수치 (Performance)가 70점대 였고 초기 페이지 로딩 시간도 약 10초 이상 소요되었으나 이를 개선하고자 ISR 을 적용한 이후로는 성능 수치 96점 달성 및 초기 페이지 로딩 시간도 2초 미만으로 단축되었습니다. 
